...
MPEG-4 and AAC encoders, MP4/MOV muxers, `pcm_f64le` and `wrapped_avframe`
decoders, and required lavfi sources and filters exist only to generate test
media. FFmpeg's `mp4` muxer selects its `mov` muxer
...
the player uses for its raw PCM audio pipe. FFmpeg's `ffmpeg` command-line program
